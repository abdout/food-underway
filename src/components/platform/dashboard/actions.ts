...
export async function checkSubdomainAvailability(subdomain: string): Promise<{
  ...
  try {
    ...
    if (existingMerchant) {
      return {
        available: false,
        error: `Subdomain already taken by "${existingMerchant.name}"`
      }
    }
...
/**
 * Update subdomain for an existing merchant
 */
...
/**
 * Get merchant data by subdomain
 */
export async function getMerchantBySubdomain(subdomain: string): Promise<{
  success: boolean
  data?: any
  error?: string
}> {
